...
>(
  ...
  const getClients = (channels = getChannels()) =>
    cachedMap(channels, (s) => createBirpc(functions, { ...options, ...s }));
  ...
  const broadcastBuiltin = {
    $call: (method: string, ...args: unknown[]) =>
      _boardcast({ method, args, event: false }),
    $callOptional: (method: string, ...args: unknown[]) =>
      _boardcast({ method, args, event: false, optional: true }),
    $callEvent: (method: string, ...args: unknown[]) =>
      _boardcast({ method, args, event: true }),
